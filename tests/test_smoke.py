...
def test_package_version_matches_pyproject():
    ...
    assert match
    assert codeglance.__version__ == match.group(1) == "0.0.4"
    readme = (root / "README.md").read_text(encoding="utf-8")
    assert "pypi/v/codeglance" not in readme
    assert "version-v0.0.4" in readme
...
def test_overview_layer_cards_and_drilldown():
    ...
    html = render_interactive(_sample_graph())
    for marker in ("drawOverview", "drawFolders", "FLL", "FOLDER_FILE_HEADER", "nodeWorldPos", "drawFolderFileFrame", "folderHasChildren", "folderBreadcrumbHTML", "goFolderParent", "enterNodeFolder", "fileParentPrefix", "attrJs", "crumb-full", "Current: ", "↑ Up", "Open a smaller folder first", "open a folder to see files", "setView", "openLayer", "pickLayer", "setCrumbHTML", "updateCrumb"):
