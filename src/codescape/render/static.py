...
def render_static_html(vm: dict) -> str:
    ...
    # edges
    parts.append('<g stroke="#6b5f53" stroke-width="1" stroke-opacity="0.5">')
    ...
    for n in nodes:
        ...
        parts.append(
            ...
            f'fill="#1a1a1a" stroke="rgba(212,165,116,0.18)"/>'
            ...
            f'<text x="{x + 10:.0f}" y="{y + ch - 14:.0f}" fill="#f5f0eb" '
...
<style>
  ...
  body {{ margin:0; background:#0a0a0a; color:#f5f0eb; font-family:ui-sans-serif,system-ui,sans-serif; }}
  header {{ padding:16px 20px; border-bottom:1px solid rgba(212,165,116,0.14); }}
  header h1 {{ margin:0; font-size:18px; }} header .sub {{ color:#a39787; font-size:13px; margin-top:3px; }}
  .wrap {{ display:flex; align-items:flex-start; }}
  .graph {{ flex:1; min-width:0; }} svg {{ width:100%; height:auto; display:block; background:#0a0a0a; }}
  aside {{ width:240px; flex:none; border-left:1px solid rgba(212,165,116,0.14); padding:16px; position:sticky; top:0; max-height:100vh; overflow:auto; }}
  aside h4 {{ margin:14px 0 8px; font-size:11px; text-transform:uppercase; letter-spacing:.06em; color:#a39787; }}
  ...
  footer {{ padding:10px 20px; color:#475569; font-size:11px; border-top:1px solid rgba(212,165,116,0.14); }}
</style></head>
<body>
...
<footer>Generated by CodeScape · static (zero-JavaScript) render.</footer>
</body></html>
"""
