...
def render_static_html(vm: dict) -> str:
    ...
    parts: list[str] = [
        ...
        '<path d="M0,0 L7,3 L0,6 Z" fill="#536b7a"/></marker></defs>'
    ]
    # containers
    for c in containers:
        parts.append(
    ...
    for n in nodes:
        ...
        parts.append(
            ...
            f'<text x="{x + 10:.0f}" y="{y + ch - 14:.0f}" fill="#e8edf2" '
...
<style>
  ...
  body {{ margin:0; background:#0a0e14; color:#e8edf2; font-family:ui-sans-serif,system-ui,sans-serif; }}
  header {{ padding:16px 20px; border-bottom:1px solid rgba(91,164,207,0.14); }}
  header h1 {{ margin:0; font-size:18px; }} header .sub {{ color:#87939f; font-size:13px; margin-top:3px; }}
  .wrap {{ display:flex; align-items:flex-start; }}
  .graph {{ flex:1; min-width:0; }} svg {{ width:100%; height:auto; display:block; background:#0a0e14; }}
  aside {{ width:240px; flex:none; border-left:1px solid rgba(91,164,207,0.14); padding:16px; position:sticky; top:0; max-height:100vh; overflow:auto; }}
  aside h4 {{ margin:14px 0 8px; font-size:11px; text-transform:uppercase; letter-spacing:.06em; color:#87939f; }}
  aside h4:first-child {{ margin-top:0; }}
  .lg {{ display:flex; align-items:center; gap:8px; padding:3px 2px; font-size:12px; }}
  .sw {{ width:11px; height:11px; border-radius:3px; flex:none; }}
  .lg .nm {{ flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }} .lg .ct {{ color:#536b7a; }}
  footer {{ padding:10px 20px; color:#536b7a; font-size:11px; border-top:1px solid rgba(91,164,207,0.14); }}
</style></head>
<body>
...
<footer>Generated by ScopingLang · static (zero-JavaScript) render.</footer>
</body></html>
"""
